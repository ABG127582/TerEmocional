...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen ${bgClass} ${textClass} p-4 md:p-8 transition-colors duration-300 font-sans relative overflow-hidden`}>
      ...
      {/* Conteúdo Principal (Sujeito a Blur quando Modal Ativo) */}
      <div className={`max-w-6xl mx-auto relative z-10 ${showSOS || showContextForm ? 'blur-sm pointer-events-none select-none' : ''} transition-all duration-300`}>
        <div className="flex justify-between items-start mb-12 gap-4 no-print">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-5 py-3 rounded-full font-bold text-white bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30 hover:scale-105 transition-all duration-300 flex items-center gap-2 animate-pulse pointer-events-auto"
            ...
            <button
              onClick={toggleTheme}
              className={`p-4 rounded-full transition-all duration-300 hover:scale-110 pointer-events-auto ${
                theme === 'dark'
      ...
      {/* Camada de Sobreposição (Modais e Toasts - Fora do Container com Blur) */}
